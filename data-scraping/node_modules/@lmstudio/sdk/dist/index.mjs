    ...
    do {
        ...
        predictImpl({
            ...
            handleToolCallGenerationEnd: request => {
                ...
                if (tool === undefined) {
                    // Tool does not exist.
                    toolCallPromises.push(internalHandleInvalidToolCallRequest(new Error(`Cannot find tool with name ${request.name}.`), request, toolCallIndex).catch(finalReject));
              ...
              `), request, toolCallIndex).catch(finalReject));
                    safeCallCallback(logger, "onToolCallRequestFailure", baseOpts.onToolCallRequestFailure, [
                        predictionsPerformed,
                        currentCallId,
                    ]);
...
function splitActOpts(opts) {
    const { onFirstToken, onPredictionFragment, onMessage, onRoundStart, onRoundEnd, onPredictionCompleted, onPromptProcessingProgress, onToolCallRequestStart, onToolCallRequestEnd, onToolCallRequestFailure, onToolCallRequestDequeued, handleInvalidToolRequest, maxPredictionRounds, signal, preset, allowParallelToolExecution, ...config } = opts;
    return [
        config,
        {
            onFirstToken,
...
class LLMDynamicHandle extends DynamicHandle {
    ...
    async act(chat, tools, opts = {}) {
        ...
        async ({ allowTools, history, signal, handleFragment, handlePromptProcessingProgress, handleToolCallGenerationStart, handleToolCallGenerationEnd, handleToolCallGenerationFailed, handlePredictionEnd, handleError, }) => {
            // Use predict channel
            const channel = this.port.createChannel("predict", {
                modelSpecifier: this.specifier,
                history,
