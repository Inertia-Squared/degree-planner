...
export default function Home() {
  ...
  const { expandConnected } = useGraphConnection({nodes, edges, setNodes, setEdges, setNodeMap, setAdjacencyList, setAddedNodes, getNodeFromId})
  const { searchProgram } = useProgram({nodes, setNodes, setSelectedProgram, setSelectedProgramSequence})
  const { forceAddSpecialisation, startExploring, selectElement, onNodeDoubleClicked, 
          moveToNewPeriod, getCompletedSubjects, isOfferedInCurrentPeriod, hasTaken 
        } = useSubjectCriteria({
          nodes, selectedProgram, setNodes, edges, setEdges, currentPeriod, setFirstShowLineup, setShowLineup, expandConnected, 
          setSelectedElement, setSelectedProgram, setSelectedProgramSequence, setClusterOptions, completedPeriods, subjectsTaken, 
          setUpdateToggle, updateToggle, adjacencyList, nodeMap, setShowSequences, setSubjectsTaken, setCurrentPeriod, setCompletedPeriods, 
          startPeriod})
  ...
  useEffect(() => {
    const { newNodes, newEdges } = nodeFilters(nodes, edges, adjacencyList, nodeMap, selectedProgram, selectedProgramSequence, showPotentialElectives, getCompletedSubjects, isOfferedInCurrentPeriod, hasTaken);
